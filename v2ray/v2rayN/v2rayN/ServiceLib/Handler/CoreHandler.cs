...
public class CoreHandler
{
    ...
    private async Task<Process?> RunProcessNormal(string fileName, CoreInfo? coreInfo, string configPath, bool displayLog)
    {
        Process proc = new()
        {
            ...
            {
                FileName = fileName,
                Arguments = "run -c stdin:", // 让核心从 stdin 读取配置
                WorkingDirectory = Utils.GetBinConfigPath(),
                UseShellExecute = false,
                RedirectStandardInput = true,     // ✅ 新增：让我们能把配置写进去
                RedirectStandardOutput = displayLog,
                RedirectStandardError = displayLog,
                CreateNoWindow = true,
                StandardOutputEncoding = displayLog ? Encoding.UTF8 : null,
            ...
            EnableRaisingEvents = true // 允许监听退出事件
        };
        ...
        var path = Utils.GetBinConfigPath(configPath);
        // ✅ 从内存写配置
        if (CoreConfigCache.Configs.TryGetValue(path, out var json))
        {
            await proc.StandardInput.WriteAsync(json.Content);
        ...
        proc.Exited += (_, _) =>
        {
            CoreConfigCache.Configs.TryRemove(path, out _);
        };
