...
public class ConfigHandler
{
    ...
    /// <param name="blImportAdvancedRules">Whether to import advanced rules</param>
    /// <returns>0 if successful</returns>
    public static async Task<int> InitBuiltinRouting(Config config, bool blImportAdvancedRules = false, string region = "cn")
    {
        var ver = "V3-";
        var items = await AppHandler.Instance.RoutingItems();
        ...
        var routingItemStr = EmbedUtils.GetEmbedText(Global.CustomRoutingFileName + "white").Replace("${geoip}", geoip).Replace("${geosite}", geosite);
        await ConfigHandler.AddBatchRoutingRules(item2, routingItemStr);
    ...
    /// <param name="config">Current configuration</param>
    /// <returns>0 if successful</returns>
    public static async Task<int> InitBuiltinDNS(Config config, string region = "cn")
    {
